...
| `gemini_acp` | `gemini` unless `cmd` overrides it | No wizard recipe. Preserve an existing verified block or require authoritative command/model/auth details. |
...
- Preserve `cmd`, `extra_args`, `model`, `mode`, `reasoning_effort`, `system_instructions`, and `mcp_servers` unless the requested change targets them.
